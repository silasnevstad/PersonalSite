...
const projects = [
  ...
  { id: '6', title: 'AlgoTrader', languages: 'Python', logo: 'algoPicksLogo.png', description: 'The algorithm starts by selecting a certain number of stocks based on certain criteria, such as recent performance and overall financial health. Next, it performs a deeper analysis on a smaller subset of those stocks to determine which ones are the best fit for the portfolio. Once the final list of stocks is determined, the algorithm calculates the appropriate allocation for each stock in the portfolio. The algorithm then compares the current portfolio to the newly generated portfolio and makes trades to align them. The algorithm will also send a text message with the list of stocks that is bought or sold. Finally, it also calculates the balance changes and sends a text message with the list of stocks that were picked. The algorithm has been implemented on a Alpaca paper trading account and has resulted in an 18% increase in overall portfolio value, it should be noted that this performance was achieved during a period where the S&P 500 (SPY) decreased by 6.14%.', developers: 'Silas Nevstad', version: '1.2.0', url: '' },
  { id: '7', title: 'VSCode GPT', languages: 'Javascript | Node.js', logo: 'vscodeLogo.png', description: 'GPTHelper is a Visual Studio Code extension that brings the power of OpenAI\'s GPT language models to your fingertips, helping you analyze and debug your code. With just a few clicks, you can get explanations for code snippets or debug outputs to help you understand what\'s going on in your code.', developers: 'Silas Nevstad', version: '2.0.1', url: 'https://marketplace.visualstudio.com/items?itemName=SilasNevstad.gpthelper' },
